...
class ConditionalGAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.which_model_netG, opt.norm, 
                                      not opt.no_dropout, self.gpu_ids, use_parallel, opt.learn_residual, opt.layers)
        if self.isTrain:
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        self.loss_G_Content = self.contentLoss.get_loss(self.fake_B, self.real_B) * self.opt.lambda_A*0.1
