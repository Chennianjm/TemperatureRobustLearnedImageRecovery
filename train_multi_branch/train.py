...
from tensorboardX import SummaryWriter   # for tensorboard
import os
...
def train(opt, data_loader, model, visualizer):
    ...
    tensorboard_root = 'tensorboard' + '\\' + opt.name
    if not os.path.exists(tensorboard_root):
        os.makedirs(tensorboard_root)
    log_writer = SummaryWriter(tensorboard_root)  # for tensorboard
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset):
            ...
            if total_steps % opt.print_freq == 0:
                ...
                log_writer.add_scalar('G_GAN', errors['G_GAN'], total_steps)  # for tensorboard
                log_writer.add_scalar('ContentLoss', errors['ContentLoss'], total_steps)  # for tensorboard
                log_writer.add_scalar('D_real+fake', errors['D_real+fake'], total_steps)  # for tensorboard
        ...
        if psnr_all/cnt > 25.:  # negelete some unimportant checkpoint
            saving_flag = 1
    ...
    log_writer.close()  # for tensorboard
